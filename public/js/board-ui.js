...
export function initBoardUI() {
  ...
  function showStampPalette() {
    ...
    const items = stampPalette.querySelector(".stamp-items");
  ...
  // ========= スタンプパレットの生成＆選択 =========
  function refreshStampPalette() {
    if (!stampPalette || !wb.stampPresets) return;
    ...
    renderStampPalette(itemsContainer, key => {
      wb.setStampType(key);
      wb.setTool("stamp");
      updateToolButtons("stamp");
      hideStampPalette();
