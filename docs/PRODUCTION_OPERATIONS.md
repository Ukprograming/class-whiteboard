...
## 生徒画面一覧の通信量（2026-09-16）

一覧用サムネイルは、教員が生徒画面確認を開いている間だけ、端末内で約500msごとに小さな比較画像を作ります。前回送信成功時から変化がなければ画像を送信しません。生徒の描画、表示位置・倍率、ページ、画像の読み込み、画面共有などの変化を検知した場合は最新画面を送信対象にします。カメラでは微細な画素差を無視しますが、暗い場所の強いノイズや照明変化で送信が続く可能性があるため、実際の教室でも確認してください。

静止後の更新には0〜2秒のランダムな待機を入れ、通常は検知周期を含めて約2.5秒以内に最初の更新を試みます。ただし、直前の送信から最低5秒は空けます。送信時にはさらに0〜750msの分散を加え、待機中の変化は最新画面1件にまとめます。タイマーの遅延やネットワーク待ちがある場合、この時間は保証されません。既存設定 `thumbnailIntervalMs` は、変更検知後にも適用する最低送信間隔として使います。

初回表示、教員が一覧へ戻ったとき、カメラ開始、モード変更、再接続、送信失敗時の再試行も同じ間隔制限を通ります。初回・再接続時は静止画面も送り直します。画像の送信に失敗した場合は成功済みとは扱わず、最新画像を後で再試行します。個別確認中の筆記差分、チャット、保存処理にはこの画像用の間引きを適用しません。

`npm.cmd test` に含まれる `scripts/test-thumbnail-sender.mjs` で、静止、再開、最新画像への集約、失敗、遅い送信中の停止・再開、30人の一斉活動を仮想時計で確認します。`node scripts/verify-thumbnail-browser.cjs` はPlaywrightとEdgeがある環境で実際の生徒ページを検証します。必要なら `PLAYWRIGHT_MODULE` に既存Playwrightの絶対パスを指定してください。ブラウザ検証の認証・通信は模擬実装です。

この制御は全クラス共通の通信上限を保証するものではありません。ランダムな時間分散には偏りがあり、画像以外の通信も合算されます。合計30人の認証済み別端末で、同時入室・同時描画開始・一斉再接続・カメラ静止と用紙変更を確認し、Realtimeのピーク件数、転送量、表示遅延を記録してください。
